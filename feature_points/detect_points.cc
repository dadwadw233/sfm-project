    ...
    detect_points::detect_points(const std::string &location) {
      ...
      std::cout<<std::endl;
        ...
        if (file_handle == -1) {
            ...
            begin_image = cv::imread(target + file_infor.name);
        ...
        std::cout << "have successfully read " << image_number << " images" << std::endl;
 ...
 */
    void detect_points::copy_key_points(std::vector<cv::Point2f> &points1,
        ...
        for (int i = 0; i < (int) matches[image_index].size(); ++i) {
            points1.push_back(begin_image_key_points[matches[image_index][i].queryIdx].pt);
            points2.push_back(key_points[image_index][matches[image_index][i].trainIdx].pt);
        }
    }
...
//                    cv::findHomography(first_image_points, now_image_points);
    ...
    void fuc(detect_points &points, std::vector<cv::Mat> &R, std::vector<cv::Mat> &t, int i) {
        points.find_feature_points(i);
        points.matchFeaturePoints(i);
        pose_estimation_2d2d(points, std::ref(R[i]), t[i], i);
    }

    void get_R_t(detect_points &points, std::vector<cv::Mat> &R, std::vector<cv::Mat> &t) {
        std::vector<std::thread> all_threads;
        for (int i = 0; i < points.image_number; ++i) {
            all_threads.push_back(std::thread{fuc, std::ref(points), std::ref(R), std::ref(t), i});
            std::cout << i << std::endl;
        }
